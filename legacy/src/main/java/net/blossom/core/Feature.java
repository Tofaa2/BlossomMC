...
public abstract class Feature {
    ...
    protected static final File DATA_FOLDER = Path.of(".").toFile();

    static void loadFeatures() {
        ServiceLoader<Feature> loader = ServiceLoader.load(Feature.class);
        for (Feature feature : loader) {
            LOGGER.info(Component.text("Discovered feature " + feature.getName()));
...
//        }
    ...
    public @Nullable BlossomCommand getCommand(@NotNull String name) {
        Command c = process().command().getCommand(name);
        if (c == null) return null;
        return (BlossomCommand) c;
    }
