...
import net.blossom.survival.world.terrain.MathHelper;
...
public class WorldGenBigTree extends WorldGenerator {
    ...
    private void a(int i, int j, int k, float f, byte b0, Block l) {
        ...
        for (aint1[b0] = aint[b0]; j1 <= i1; ++j1) {
            ...
            while (k1 <= i1) {
                ...
                if (d0 > (double) f) {
                    ...
                    aint1[b2] = aint[b2] + k1;
                    Block l1 = chunk.getInstance().getBlock(aint1[0], aint1[1], aint1[2], Block.Getter.Condition.TYPE);

                    if (!l1.isAir() && !l1.namespace().value().contains("_leaves")) {
                        ++k1;
                    } else {
                        chunk.getInstance().setBlock(aint1[0], aint1[1], aint1[2], l);
                        ++k1;
                    }
                }
            }
    ...
    private void a(int i, int j, int k) {
        ...
        for (int i1 = j + this.n; l < i1; ++l) {
            ...
            this.a(i, l, k, f, (byte) 1, Block.OAK_LEAVES);
        }
    }

    private void a(int[] aint, int[] aint1, Block i) {
        ...
        if (aint2[b1] != 0) {
            ...
            for (int k = aint2[b1] + b4; j != k; j += b4) {
                ...
                chunk.getInstance().setBlock(aint3[0], aint3[1], aint3[2], i);
            }
        }
    }
    ...
    private void c() {
        ...
        this.a(aint, aint1, Block.OAK_LOG);
        if (this.l == 2) {
            ++aint[0];
            ++aint1[0];
            this.a(aint, aint1, Block.OAK_LOG);
    ...
    private void placeLogs() {
        ...
        for (int[] aint = new int[] { this.pos[0], this.pos[1], this.pos[2]}; i < j; ++i) {
            ...
            if (this.c(k)) {
                this.a(aint, aint2, Block.OAK_LOG);
            }
        }
    }
    ...
    private int a(int[] aint, int[] aint1) {
        ...
        if (aint2[b1] == 0) {
            ...
            for (j = aint2[b1] + b4; i != j; i += b4) {
                ...
                Block k = chunk.getInstance().getBlock(aint3[0], aint3[1], aint3[2], Block.Getter.Condition.TYPE);

                if (!k.isAir() && !k.namespace().value().contains("_leaves")) {
                    break;
                }
            }
    ...
    private boolean d() {
        ...
        Block i = chunk.getInstance().getBlock(this.pos[0], this.pos[1] - 1, this.pos[2], Block.Getter.Condition.TYPE);

        if (i != Block.GRASS_BLOCK && i != Block.DIRT) {
            return false;
        } else {
