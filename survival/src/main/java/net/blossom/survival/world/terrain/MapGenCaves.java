...
public class MapGenCaves extends MapGenBase {
    ...
    public MapGenCaves(long seed, GeneratorAssistance generatorAssistance) {
        super(seed);
        this.generatorAssistance = generatorAssistance;
    }

    protected void generate(int i, int j, GenerationUnit chunkData, double d0, double d1, double d2) {
        this.generate(i, j, chunkData, d0, d1, d2, 1.0F + this.random.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void generate(int i, int j, GenerationUnit chunkData, double d0, double d1, double d2, float f, float f1, float f2, int k, int l, double d3) {
        ...
        for (boolean flag1 = random.nextInt(6) == 0; k < l; ++k) {
            ...
            if (!flag && k == j1 && f > 1.0F) {
                this.generate(i, j, chunkData, d0, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 - 1.5707964F, f2 / 3.0F, k, l, 1.0D);
                this.generate(i, j, chunkData, d0, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 + 1.5707964F, f2 / 3.0F, k, l, 1.0D);
                return;
            }
                    ...
                    for (j3 = k1; !flag2 && j3 < l1; ++j3) {
                        for (int k3 = k2; !flag2 && k3 < l2; ++k3) {
                            for (int l3 = j2 + 1; !flag2 && l3 >= i2 - 1; --l3) {
                                i3 = (j3 * 16 + k3) * 128 + l3;
                                if (l3 >= 0 && l3 < 128) {
                                    if (generatorAssistance.getType(
                                            chunkData, i3
                                    ) == Block.WATER)
                                        flag2 = true;
                                ...
                                if (d12 * d12 + d13 * d13 < 1.0D) {
                                    for (int j4 = j2 - 1; j4 >= i2; --j4) {
                                        ...
                                        if (d14 > -0.7D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D) {
                                            Block b0 = generatorAssistance.getType(chunkData, i4);
                                            ...
                                            if (b0 == Block.STONE || b0 == Block.DIRT || b0 == Block.GRASS_BLOCK) {
                                                if (j4 < 10) {
                                                    generatorAssistance.setBlock(chunkData, i4, Block.LAVA);
                                                } else {
                                                    generatorAssistance.setBlock(chunkData, i4, Block.AIR);
                                                    if (flag3 && generatorAssistance.getType(chunkData, i4 - 1) == Block.DIRT) {
                                                        generatorAssistance.setBlock(chunkData, i4 - 1, Block.GRASS_BLOCK);
                                                    }
                                                }
                                            }
                                        }
    ...
    protected void generate(Instance instance, int i, int j, int k, int l, GenerationUnit chunkData) {
        ...
        for (int j1 = 0; j1 < i1; ++j1) {
            ...
            if (this.random.nextInt(4) == 0) {
                this.generate(k, l, chunkData, d0, d1, d2);
            ...
            for (int l1 = 0; l1 < k1; ++l1) {
                ...
                this.generate(k, l, chunkData, d0, d1, d2, f2, f, f1, 0, 0, 1.0D);
            }
        }
    }
}